...
function Game() {
  ...
  const backToRoomPage = async () => {
    socket.disconnect();
    await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/userrooms/leave`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-4 h-screen bg-zinc-700">
      ...
      <div className="flex justify-center relative">
        <h1 className="absolute top-0 left-[30%] text-2xl opacity-[50%] sm:text-9xl">
          {leftSideScore}
        </h1>
        <canvas ref={canvasRef} width={width} height={height} />
        <h1 className="absolute top-0 right-[30%] text-2xl opacity-[50%] sm:text-9xl">
          {rightSideScore}
        </h1>
